...
export default function TeamPortalPage() {
  ...
  async function loadDashboard() {
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        // stats.* is no longer read here: the four cards it fed were
        // replaced by the dashboard's own derived counters. The chart,
        // pipeline and top matches below still come from this call.
  ...
  return (
    ...
    <div>
      ...
      {/* ═══ DASHBOARD (Atlas steps 5 + 14) ═══
          The welcome, the attention queue, engagements, spend and activity —
          two states, switched on whether the account can fund yet.
          It replaces the old page header and the four stat cards, which
          counted things ("candidates contacted", "total spend") without
          saying what to do about any of them; the three counters it keeps
          are the ones tied to an action. What follows — interviews, the
          hiring chart, offers, roles, engagements, escrow — is still the
          working detail the rail deep-links to, until each step moves it
          onto its own page. */}
      <ClientDashboard />
