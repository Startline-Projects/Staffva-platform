 ...
 * (`client_notifications`); the behavior is identical, so it lives here once rather than
 * drifting in two copies.
 *
 * Faithful to the prototype: badge with unread count, dropdown with
 * All/Unread filter pills, mark-all-read (disabled at zero), click marks one
 * read and routes, outside-click and Escape close. Counts are DERIVED from
 * the data — the prototype hard-coded "3" against four seeded unread items,
 * a bug its own spec flags.
 *
 * Routes come from the database but were written by our own server code and
 * both tables CHECK-constrain them to app-relative paths; the guard here is
...
const CAT_ICONS: Record<NotifCategory, React.ReactNode> = {
  offer: (
  ...
  payment: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
      <rect x="1.5" y="3.5" width="11" height="7.5" rx="1.2" stroke="currentColor" strokeWidth="1.4" />
  ...
  engagement: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
      ...
      <path d="M1.5 12c.4-2 2-3.1 3.5-3.1S8.1 10 8.5 12M9.5 4.2a2.2 2.2 0 0 1 0 4.2" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
    </svg>
  ),
  interview: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
...
export default function PortalNotifBell({
  ...
  /** GET returns {notifications, unread}; POST marks read. */
  endpoint: string;
  ...
  function onItemClick(n: Notif) {
    ...
    // rejects it at write time and this guard rejects it at click time,
    // because a stored row outlives whichever layer was fixed last.
    if (
      n.route &&
      n.route.startsWith("/") &&
      !n.route.startsWith("//") &&
        ...
        <div className="notif-list">
          {shown.length === 0 ? (
            <p style={{ padding: "22px 18px", fontSize: 13, color: "var(--ink-mute)" }}>
              {filter === "unread" ? "You're all caught up." : emptyText}
            </p>
          ) : (
            shown.map((n) => (
              <button
