-- Interface-audit fix: search returned "No matches" for the platform's most
-- common queries. Two causes, both here:
--  * The browse pills and AI parser speak a coarse vocabulary ("VA",
--    "Bookkeeping/AP") that no candidates.role_category row contains, and
--    the old substring filter (role ILIKE '%VA%'... actually matched
--    nothing useful either way). p_role text becomes p_roles text[] of
--    ILIKE patterns; the API layer maps each pill to the stored role
--    values it covers.
--  * The skills filter (skills @> to_jsonb(p_skills)) required exact
--    case-sensitive containment in skills ONLY — "knows QuickBooks" and
--    "knows Clio" matched nothing because clients name TOOLS. Requested
--    terms now match case-insensitive substrings across skills AND tools.
...
-- Same posture as 00137: the only caller is the API route's service client.
REVOKE EXECUTE ON FUNCTION get_candidates_with_skills(
  text, text[], text, numeric, numeric, text, text, text, text[], text, integer, integer
