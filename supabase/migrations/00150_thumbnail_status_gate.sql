-- Review follow-up to 00148: the thumbnail must not ship before (or after)
-- the video it belongs to clears review. 00148 returned
-- video_intro_thumbnail_url unconditionally, so a pending or REJECTED
-- video's thumbnail would still sit in the public /api/candidates JSON —
-- the UI hides it, but scrapers read the JSON, not the page. Gate it on
-- video_intro_status = 'approved' in both SELECT lists.
--
-- Note on the column's semantics (decided here, enforced app-side): the
-- value must be a server-derived, publicly fetchable image URL. The client
-- -supplied thumbnailUrl body field was dropped from
-- /api/candidate/video-intro in the same change — nothing writes this
-- column until server-side derivation exists, and the browse card falls
-- back to the profile photo, which is the required behavior for the (many)
-- candidates who never upload a video at all.
