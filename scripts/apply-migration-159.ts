/**
 * Apply migration 00159 — revoke dangerous table grants from anon/authenticated.
 *
 * Revokes TRUNCATE/REFERENCES/TRIGGER/MAINTAIN everywhere, DELETE from anon
 * everywhere, and DELETE from authenticated except the nine tables with a
 * scoped delete policy; trims the postgres default ACL the same way.
 *
 ...
 *
 * Run: npx tsx --env-file=.env.local scripts/apply-migration-159.ts
 */
...
async function main() {
  const migrationPath = join(
    process.cwd(),
    "supabase/migrations/00159_revoke_dangerous_table_grants.sql"
  );
  console.log("════════════════════════════════════════════════════════════");
  console.log("Applying migration 00159");
  ...
  console.log(`✅ Migration 00159 applied in ${elapsedMs}ms`);
  console.log("Next: npx tsx --env-file=.env.local scripts/audit-table-grants.ts");
}
